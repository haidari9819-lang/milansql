...
public:
    ...
    void commitTx(uint64_t id) {
        ...
        // Bug #22: Prune old committed TxIds to prevent unbounded memory growth.
        // Any txId below the minimum active transaction is no longer needed
        // for visibility checks (all active snapshots can already see it).
        if (committedTxs_.size() > 10000) {
            uint64_t horizon = minActiveTxId();
            auto it = committedTxs_.begin();
            while (it != committedTxs_.end() && *it < horizon)
                it = committedTxs_.erase(it);
        }
    }
    ...
    // Bug #22: After pruning, IDs below minActiveTxId that are not in
    // activeTxs_ are implicitly committed (they were pruned from committedTxs_).
    bool isCommitted(uint64_t id) const {
        if (id == 0) return true;
        if (committedTxs_.count(id) > 0) return true;
        // If not active and below the horizon, it was pruned → committed
        if (activeTxs_.count(id) == 0 && id < minActiveTxId()) return true;
        return false;
    }
