...
inline std::string MilanHttpServer::handleQueryForUser(const std::string& sql, int userId, const std::string& userRole) {
    ...
    // Handle: SELECT @@version, SELECT @@version_comment, SELECT @@global.version, etc.
    {
        ...
        if (u2 == "SELECT @@VERSION" || u2 == "SELECT @@GLOBAL.VERSION")
            return makeScalar("@@version", "9.6.0");
        ...
        if (u2 == "SELECT @@VERSION, @@VERSION_COMMENT" ||
            ...
            return "{\"success\":true,\"columns\":[\"@@version\",\"@@version_comment\"],\"rows\":[[\"9.6.0\",\"MilanSQL Database Engine\"]]}";
...
<body>
<div class="header">
  <div class="logo">&#9889; MilanSQL v9.6.0</div>
...
<div id="topbar">
  ...
  <div class="topbar-right">
    ...
    <span style="font-size:0.75rem;color:#8b949e" id="version-label">v9.6.0</span>
  </div>
</div>
...
<div id="layout">
  ...
  <nav id="sidebar">
    ...
    <div class="sidebar-footer">MilanSQL Admin v9.6.0</div>
  </nav>
...
<div id="statusbar">
  ...
  <div class="status-item" style="margin-left:auto;font-size:0.7rem;color:#484f58">MilanSQL v9.6.0 &middot; Press Ctrl+Enter to run</div>
</div>
...
<div id="ms-login-overlay" style="display:flex;position:fixed;top:0;left:0;width:100%;height:100%;background:#11111b;z-index:9999;justify-content:center;align-items:center;">
  <div style="background:#1e1e2e;border:1px solid #313244;border-radius:16px;padding:0;width:360px;box-shadow:0 16px 48px rgba(0,0,0,0.7);overflow:hidden">
    ...
    <div style="background:#181825;padding:28px 32px 20px;text-align:center;border-bottom:1px solid #313244">
      ...
      <div style="color:#585b70;font-size:11px;margin-top:4px">v9.6.0 &mdash; Multi-User Database</div>
    </div>
    <!-- Tabs -->
